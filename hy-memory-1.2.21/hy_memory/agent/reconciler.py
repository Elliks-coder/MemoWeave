...
流程:
  ...
  2. 候选搜索: 默认每条新 memory 做 hybrid 检索（向量池 limit×3 + BM25，min_score=0.3）；
    ...
    view = [
        {
            ...
            "source_indices": list(o.source_indices or []),
            "temporal_relation": o.temporal_relation,
...
class ReconcileOp:
    ...
    op = "UPDATE"：合并与精炼。新记忆与旧记忆描述同一主题且不矛盾，合并为高密度节点。
      - memory_id 指向被更新的旧节点
      - content 是合并后的完整文本
      - 执行：原地更新 content / embedding / tags / temporal metadata
      - 保留原 node_id、首次 observed_at、valid_from 与访问统计，不形成演化链
    """
    ...
    # --- L2 temporal schema（ISO 字符串保留到 writer 再统一解析）---
...
* Every operation MUST include `source_indices`, referring to the `source_index`
  values of the new memories consumed by that operation. Preserve their temporal
  fields; never merge distinct episodic events merely because their topics match.
* UPDATE is only for compatible enrichment of the same state/event. A change in
  a time-varying state (location, employer, relationship, schedule, active plan,
  preference reversal) MUST use SUPERSEDE so the old validity interval remains
  queryable. If uncertain whether both states can be simultaneously true, do not
...
class MemoryReconciler:
    ...
    # Reconcile 需要偏重召回，避免重复事实因读取阈值过严而完全看不到旧节点。
    # 与面向用户的 search 阈值分离，并允许部署按模型分数分布调节。
    HYBRID_MIN_SCORE = float(os.getenv("MEMORY_RECONCILE_MIN_SCORE", "0.3"))
    ...
    async def reconcile(
        ...
        session_id: str = "",  # 隔离维度，透传到 pipeline_logs
    ) -> ReconcileResult:
        """
        ...
        try:
            ...
            if not candidate_map:
                ...
                if new_memories_with_meta:
                    for source_index, meta in enumerate(new_memories_with_meta):
                        all_add_ops.append(
                            ReconcileOp(
                                op="ADD",
                                ...
                                temporal_relation=meta.get("temporal_relation"),
                                event_time_text=meta.get("event_time_text"),
                                event_start=meta.get("event_start"),
                                event_end=meta.get("event_end"),
                                normalization_confidence=meta.get("normalization_confidence"),
            ...
            if new_memories_with_meta:
                ...
                for meta in new_memories_with_meta:
                    ...
                    for key in (
                        "observed_at", "temporal_relation", "event_time_text",
                        "event_start", "event_end", "normalization_confidence",
                ...
                new_mem_list = [{"source_index": i, "content": m, "owner": "user", "memory_at": _format_memory_at(current_time) if current_time else None, "tags": []} for i, m in enumerate(new_memories)]
                new_mem_lines = json.dumps(new_mem_list, ensure_ascii=False, indent=2)
            ...
            for node in sorted_candidates:
                item: Dict[str, Any] = {
                    ...
                    "observed_at": _format_memory_at(getattr(node, "observed_at", None)),
                    "temporal_relation": getattr(node, "temporal_relation", None),
                    "event_time_text": getattr(node, "event_time_text", None),
                    "event_start": _format_memory_at(getattr(node, "event_start", None)),
                    "event_end": _format_memory_at(getattr(node, "event_end", None)),
                    "normalization_confidence": getattr(node, "normalization_confidence", None),
                    "valid_from": _format_memory_at(getattr(node, "valid_from", None)),
                    "valid_until": _format_memory_at(getattr(node, "valid_until", None)),
                }
                if getattr(node, "tags", None):
            ...
            # JSON 解析失败只重试一次（首次 + 1 次重试 = 共 2 次）。
            # 配合 _parse_ops 的 json_repair 兜底，多数残缺输出首次即可修复。
            _temperatures = [_base_temp, _base_temp]
            ...
            for _attempt, _temp in enumerate(_temperatures):
                ...
                self._enrich_ops_temporal(ops, new_memories_with_meta or [])
                ...
                if ops or _is_empty_array:
                    ...
                    break
                logger.warning(
                    f"[reconciler] RECONCILE JSON parse failed "
                    f"(attempt {_attempt+1}/{len(_temperatures)}), "
            ...
            # ── Step 6: 解析仍失败 ──
            if not _parse_ok:
                _n_attempts = len(_temperatures)
                error_msg = (
                    f"RECONCILE JSON parse failed after {_n_attempts} attempts: {raw_response[:200]}"
                    if raw_response.strip()
                    else f"RECONCILE LLM returned empty response after {_n_attempts} attempts"
                )
                error_code = (
                    "EMPTY_RESPONSE"
                            ...
                            [
                                {
                                    ...
                                    "temporal_relation": o.temporal_relation,
                                    "event_time_text": o.event_time_text,
                                    "event_start": o.event_start,
                                    "event_end": o.event_end,
                                    "normalization_confidence": o.normalization_confidence,
    ...
    @staticmethod
    def _enrich_ops_temporal(
        ops: List[ReconcileOp],
        new_memories_with_meta: List[Dict[str, Any]],
    ) -> None:
        """Attach extractor temporal metadata using explicit source indices.

        The LLM only needs to return ``source_indices``; it does not have to
        reproduce every ISO value.  Exact-content and single-input fallbacks
        keep older/custom reconcile prompts backward compatible.
        """
        if not ops or not new_memories_with_meta:
            return
        temporal_fields = (
            "observed_at", "temporal_relation", "event_time_text",
            "event_start", "event_end", "normalization_confidence",
        ...
        for op in ops:
            indices = [
                i for i in (op.source_indices or [])
                if 0 <= i < len(new_memories_with_meta)
            ]
            if not indices and op.content:
                exact = [
                    i for i, meta in enumerate(new_memories_with_meta)
                    if meta.get("content") == op.content
                ]
                indices = exact
            if not indices and len(new_memories_with_meta) == 1:
                indices = [0]
            op.source_indices = indices
            sources = [new_memories_with_meta[i] for i in indices]
            for field_name in temporal_fields:
                if getattr(op, field_name) is not None:
                    continue
                ...
                for value in values:
                    if value not in unique:
                        unique.append(value)
                # Conflicting intervals must remain explicit/unknown rather
                # than silently selecting one source and corrupting chronology.
                if len(unique) == 1:
                    setattr(op, field_name, unique[0])
    ...
    def _parse_ops(self, text: str) -> List[ReconcileOp]:
        ...
        try:
            ...
            if match:
                try:
                    ...
                    data = None
            if data is None:
                # 兜底：json_repair 修复常见 LLM 残缺输出（缺逗号/引号/未闭合等）
                try:
                    from json_repair import repair_json

                    repaired = repair_json(text, return_objects=True)
                    if repaired not in (None, "", {}):
                        logger.info("[reconciler] JSON recovered via json_repair")
                        data = repaired
                except Exception as _re:
                    logger.debug(f"[reconciler] json_repair failed: {_re}")
            if data is None:
                logger.warning(f"[reconciler] failed to parse ops JSON: {text[:200]}")
                return []
        ...
        for op_dict, group_reason in flat_items:
            ...
            raw_source_indices = op_dict.get("source_indices") or []
            ...
            for raw_index in raw_source_indices:
                try:
                    ...
                    continue
                if index >= 0 and index not in source_indices:
                    source_indices.append(index)

            raw_confidence = op_dict.get("normalization_confidence")
            normalization_confidence = None
            if raw_confidence is not None:
                try:
                    normalization_confidence = max(0.0, min(1.0, float(raw_confidence)))
                except (TypeError, ValueError):
                    normalization_confidence = None
            state_transition = bool(op_dict.get("state_transition", False))
            temporal_kwargs = {
                "observed_at": op_dict.get("observed_at"),
                "temporal_relation": op_dict.get("temporal_relation"),
                "event_time_text": op_dict.get("event_time_text"),
                "event_start": op_dict.get("event_start"),
                "event_end": op_dict.get("event_end"),
                "normalization_confidence": normalization_confidence,
                "valid_from": op_dict.get("valid_from"),
                "valid_until": op_dict.get("valid_until"),
                "source_indices": source_indices,
                "state_transition": state_transition,
            }

            # A model occasionally labels an operation UPDATE while explicitly
            # acknowledging that it changes the current state.  Enforce the
            # evolution invariant at the parser boundary rather than allowing
            # the writer to destroy the old state in place.
            if op_type == "UPDATE" and state_transition:
                op_type = "SUPERSEDE"
                op_dict.setdefault(
                    "supersede_reason",
                    op_reason or "time-varying state transition",
                )
