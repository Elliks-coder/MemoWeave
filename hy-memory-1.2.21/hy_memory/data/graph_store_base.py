...
class GraphStoreBase(ABC):
    ...
    async def list_schema_embeddings(
        ...
        """Return active L6 schemas with normalized content/behaviour vectors.

        Backends must return dictionaries containing ``node_id``, ``content``,
        ``embedding`` and ``beh_embedding``.  Raising here is intentional: an
        unsupported backend must not be misreported by the sweeper as a valid
        scan with zero collisions.
        """
        raise NotImplementedError(
            f"{type(self).__name__} does not implement list_schema_embeddings"
        )
