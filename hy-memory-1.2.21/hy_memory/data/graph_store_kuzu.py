...
# Kuzu 专属单线程 executor。
# 关键约束：
#   1) Kuzu 的 Connection 不是线程安全的 —— 并发在同一连接上 execute 会数据损坏/崩溃。
#   2) Kuzu 的 QueryResult 有线程亲和性 —— execute 与结果消费（has_next/get_next）
#      必须在同一线程完成。
# 因此用 max_workers=1 的单线程池：既把同步的 Kuzu 调用挪出 event loop（不再阻塞
# 整个 server 的唯一 loop），又天然串行化所有 Kuzu 操作，满足上面两条约束。
# 绝不要调大 max_workers。
_KUZU_POOL_SIZE = 1
_kuzu_executor = concurrent.futures.ThreadPoolExecutor(
    max_workers=_KUZU_POOL_SIZE, thread_name_prefix="kuzu"
)


def _run_in_kuzu_pool(func, *args, **kwargs):
    """在 Kuzu 单线程池中执行同步函数（execute + 结果消费必须整体在此运行）。"""
    loop = asyncio.get_event_loop()
    if args or kwargs:
        return loop.run_in_executor(_kuzu_executor, functools.partial(func, *args, **kwargs))
    return loop.run_in_executor(_kuzu_executor, func)


def _materialize_row(row: Any) -> List[Any]:
    """把 Kuzu 结果行物化成跨线程安全的普通 list。

    行内元素可能是：
      - Kuzu Node 对象（RETURN m 时 row[0]）→ 转成 dict（_row_to_memory_node 能吃 dict）
      - 标量（RETURN m.x, m.y 时的各列）→ 原样保留
    物化后脱离 QueryResult，可安全跨线程传回 event loop。
    """
    out: List[Any] = []
    for col in row:
        if isinstance(col, dict) or isinstance(col, (str, int, float, bool)) or col is None:
            out.append(col)
        elif hasattr(col, "__iter__") and not isinstance(col, (list, tuple)):
            # Kuzu Node / Rel 对象（dict-like）
            try:
                out.append(dict(col))
            except Exception:
                out.append(col)
...
class KuzuGraphStore(GraphStoreBase):
    ...
    def _execute_materialized(
        ...
        """【必须在 kuzu 单线程 executor 内调用】执行 query 并把 QueryResult 完整
        物化成跨线程安全的 List[row]（row 为 list，Node 元素转 dict）后返回。

        execute 与 has_next/get_next 消费在同一 worker 线程完成（QueryResult 线程
        亲和）。返回的 list 已脱离 QueryResult，可安全传回 event loop。
        无结果的写语句返回空 list；GraphStore 不可用返回 None。
        """
        ...
        try:
            while result.has_next():
                rows.append(_materialize_row(result.get_next()))
        except Exception:
            # 写语句可能没有可迭代结果集
            pass
        return rows
    ...
    async def _run(
        ...
        """异步执行 Cypher：把同步的 Kuzu 调用卸载到单线程 executor，不阻塞 event loop。

        返回物化后的 List[row]（row 为 list）；不可用时返回 None。
        写语句忽略返回值即可。
        """
        if not self._available or self._conn is None:
            return None
        return await _run_in_kuzu_pool(self._execute_materialized, query, params)

    def _execute(self, query: str, params: Optional[Dict[str, Any]] = None) -> Any:
        """【已废弃/仅 init 用】同步执行，直接跑在调用线程上。

        运行期严禁调用（会阻塞 event loop，正是死锁根因）。仅 initialize() 期间、
        服务尚未对外时使用。运行期请一律用 await self._run(...)。
        """
        if not self._available or self._conn is None:
            return None
        if params:
    ...
    def _table_exists(self, table_name: str) -> bool:
        """检查 Kuzu 中某个 node/rel table 是否存在（仅 init 期同步调用）
    ...
    async def upsert_memory_node(self, node: MemoryNode) -> str:
        ...
        # Kuzu 的 Memory 表在已有用户库中是固定 schema。时间扩展字段
        # 放入已预留的 extra_json，避免要求用户重建整个图库。
        temporal_extra = {
            "memory_at": _iso("memory_at"),
            ...
            "normalization_confidence": getattr(
                node, "normalization_confidence", None
            ),
        ...
        params = {
            ...
            "extra": json.dumps(temporal_extra, ensure_ascii=False),
        }
    ...
    async def update_node(self, node_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            rows = await self._run(
                ...
                f"SET {set_str} RETURN m.node_id;",
                params,
            )
            return bool(rows and rows[0][0] == node_id)
        except Exception as e:
    ...
    async def delete_by_metadata(
        ...
        try:
            ...
            mem_count = count_rows[0][0] if count_rows else 0
            await self._run(
    ...
    async def add_edge(
        ...
        try:
            ...
            # Kuzu treats a zero-row MATCH as a successful query.  Check both
            # endpoints and tenant isolation explicitly before mutating.
            endpoint_rows = await self._run(
                """
                MATCH (a:Memory {node_id: $src}), (b:Memory {node_id: $tgt})
                RETURN a.node_id, b.node_id, a.isolation_key, b.isolation_key;
                """,
                {"src": source_id, "tgt": target_id},
            )
            if not endpoint_rows or endpoint_rows[0][2] != endpoint_rows[0][3]:
                return False

            async def _edge_count(src: str, tgt: str) -> int:
                rows = await self._run(
                    """
                    MATCH (a:Memory {node_id: $src})-[r:RELATED_TO]->
            ...
            # Idempotent repair: create only the missing direction.  This also
            # heals old partially-written pairs.
            if await _edge_count(source_id, target_id) == 0:
                ...
                if not created:
                    return False
            if await _edge_count(target_id, source_id) == 0:
                reverse_params = dict(params)
                reverse_params["src"], reverse_params["tgt"] = target_id, source_id
                created = await self._run(
                    """
                    MATCH (a:Memory {node_id: $src}), (b:Memory {node_id: $tgt})
                    CREATE (a)-[:RELATED_TO {
            ...
            return (
                await _edge_count(source_id, target_id) > 0
                and await _edge_count(target_id, source_id) > 0
            )
        except Exception as e:
    ...
    async def add_topic_tag(
        ...
        try:
            ...
            topic_rows = await self._run(
                "MATCH (t:Topic {topic_id: $tid}) RETURN t.topic_id;",
                {"tid": topic_id}
            )
            topic_exists = bool(topic_rows)
    ...
    async def list_schema_embeddings(self, isolation_key: str) -> List[Dict[str, Any]]:
        ...
        if not self._available:
            raise RuntimeError("Kuzu graph store is unavailable")
        rows = await self._run(
            """
            MATCH (m:Memory)
        ...
        if rows is None:
            raise RuntimeError("Kuzu schema embedding query returned no result object")
        return [
            {
                "node_id": row[0],
                "content": row[1],
    ...
    async def update_embedding(
        ...
        try:
            if beh_embedding is not None:
                rows = await self._run(
                    "MATCH (m:Memory {node_id: $nid}) "
                    "SET m.beh_embedding = $vec "
                    "RETURN m.node_id, m.beh_embedding;",
                    {"nid": node_id, "vec": beh_embedding},
                )
                if not rows or rows[0][0] != node_id or rows[0][1] is None:
                    return False
            if embedding is not None:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            mem_rows = await self._run("MATCH (m:Memory) RETURN count(m);")
            user_rows = await self._run("MATCH (u:User) RETURN count(u);")
    ...
    def _row_to_memory_node(row_data: Any) -> MemoryNode:
        ...
        return MemoryNode.from_dict({
            ...
            "memory_at": extra.get("memory_at"),
            "observed_at": extra.get("observed_at"),
            "temporal_relation": extra.get("temporal_relation"),
            "event_time_text": extra.get("event_time_text"),
            "event_start": extra.get("event_start"),
            "event_end": extra.get("event_end"),
            "normalization_confidence": extra.get("normalization_confidence"),
            "access_count": d.get("access_count", 0),
            "created_at": d.get("created_at"),
            "valid_from": d.get("valid_from") or extra.get("valid_from"),
            "valid_until": d.get("valid_until") or extra.get("valid_until"),
