...
存储的 payload 结构:
  ...
  - status: str
  - confidence: float
  - memory_at / observed_at / temporal_anchor / gmt_created / gmt_modified: str (ISO) | None
  - temporal_relation / event_time_text: str | None
  - event_start / event_end: str (ISO) | None
...
class VectorStoreBase(ABC):
    ...
    def _node_to_payload(node: MemoryNode) -> Dict[str, Any]:
        ...
        return {
            ...
            "source_turn_index": node.source_turn_index,
            "evidence_chain": list(node.evidence_chain or []),
            "memory_at": int(node.memory_at.timestamp()) if node.memory_at else None,
            "observed_at": int(node.observed_at.timestamp()) if node.observed_at else None,
            "temporal_relation": node.temporal_relation,
            "event_time_text": node.event_time_text,
            "event_start": int(node.event_start.timestamp()) if node.event_start else None,
            "event_end": int(node.event_end.timestamp()) if node.event_end else None,
            "normalization_confidence": node.normalization_confidence,
