...
class MetricsCollector:
    ...
    async def start_background_tasks(self) -> None:
        ...
        self._background_tasks = [
            asyncio.create_task(self._flush_loop(), name="hy-memory-metrics-flush"),
            asyncio.create_task(self._cleanup_loop(), name="hy-memory-metrics-cleanup"),
        ]
        logger.info("[metrics] background tasks started (flush=60s, cleanup=3600s)")

    async def stop_background_tasks(self, *, flush: bool = True) -> None:
        """停止后台任务，并在存储关闭前尽力落盘最后一个指标桶。"""
        ...
        if flush:
            try:
                ...
                logger.warning(f"[metrics] final flush error: {e}")
