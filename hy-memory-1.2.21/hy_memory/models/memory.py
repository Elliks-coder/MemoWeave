...
class MemoryNode:
    ...
    # === 时空维度 ===
    memory_at: Optional[datetime] = None     # 兼容字段：对话/记忆观测时间（用户指定，可为空）
    observed_at: Optional[datetime] = None   # 明确的观测时间；与事件实际发生时间分离
    temporal_relation: Optional[str] = None  # past / present / future / recurring / atemporal / unknown
    event_time_text: Optional[str] = None    # 原始时间表达，如 "this summer" / "上周五"
    event_start: Optional[datetime] = None   # 事件时间点或时间区间起点
    event_end: Optional[datetime] = None     # 事件时间区间终点
    normalization_confidence: Optional[float] = None  # 时间标准化置信度 [0, 1]
    ...
    def __post_init__(self):
        ...
        # L2/L7 将观测时间与事件时间分开；旧调用方只传 memory_at 时自动兼容。
        if (
            self.observed_at is None
            and self.layer in (MemoryLayer.L2_FACT, MemoryLayer.L7_INTENTION)
        ):
            self.observed_at = self.memory_at
        if self.temporal_relation:
            relation = str(self.temporal_relation).strip().lower()
            allowed = {"past", "present", "future", "recurring", "atemporal", "unknown"}
            self.temporal_relation = relation if relation in allowed else "unknown"
        if self.normalization_confidence is not None:
            try:
                self.normalization_confidence = max(
                    0.0, min(1.0, float(self.normalization_confidence))
                )
            except (TypeError, ValueError):
                self.normalization_confidence = None
        # memory_at/observed_at 由调用方指定，不自动填写
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "observed_at": int(self.observed_at.timestamp()) if self.observed_at else None,
            "temporal_relation": self.temporal_relation,
            "event_time_text": self.event_time_text,
            "event_start": int(self.event_start.timestamp()) if self.event_start else None,
            "event_end": int(self.event_end.timestamp()) if self.event_end else None,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "MemoryNode":
        ...
        return cls(
            ...
            observed_at=parse_dt(data.get("observed_at")),
            temporal_relation=data.get("temporal_relation"),
            event_time_text=data.get("event_time_text"),
            event_start=parse_dt(data.get("event_start")),
            event_end=parse_dt(data.get("event_end")),
            normalization_confidence=data.get("normalization_confidence"),
