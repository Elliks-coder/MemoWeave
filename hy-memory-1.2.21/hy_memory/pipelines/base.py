...
def memory_temporal_fields(node: Any) -> Dict[str, Any]:
    """Serialize the public L2/L7 temporal schema for read/list responses."""
    def _epoch(name: str):
        value = getattr(node, name, None) if node is not None else None
        return int(value.timestamp()) if isinstance(value, datetime) else None

    return {
        "observed_at": _epoch("observed_at"),
        "temporal_relation": getattr(node, "temporal_relation", None) if node else None,
        "event_time_text": getattr(node, "event_time_text", None) if node else None,
        "event_start": _epoch("event_start"),
        "event_end": _epoch("event_end"),
        "normalization_confidence": getattr(node, "normalization_confidence", None) if node else None,
        "valid_from": _epoch("valid_from"),
        "valid_until": _epoch("valid_until"),
    }


def memory_provenance_fields(node: Any) -> Dict[str, Any]:
    """Serialize source-turn provenance for public read/search responses."""
    evidence_chain = getattr(node, "evidence_chain", None) if node else None
    return {
        "source_session_id": getattr(node, "source_session_id", "") if node else "",
        "source_turn_index": getattr(node, "source_turn_index", None) if node else None,
...
class ChatMessage:
    ...
    # 原始对话 turn 的稳定标识。SDK 输入兼容 turn_id / dia_id / message_id，
    # 统一落到此字段并沿 extractor -> L2/L7 evidence_chain 透传。
    turn_id: str = ""
...
class ReadRequest:
    ...
    intention_min_score: float = 0.4     # Intention 路的最低分数阈值
    intention_limit: int = 10          # Intention 路的 topk，默认召回；0 显式关闭


    # 时间过滤
    created_after: Optional[float] = None  # Unix timestamp (float)，只返回 gmt_created >= 此值的记忆
    as_of: Optional[datetime] = None       # 查询视角时间；L7 过期判断默认使用当前时间
