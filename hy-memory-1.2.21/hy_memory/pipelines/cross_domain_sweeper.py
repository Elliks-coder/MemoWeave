...
class CrossDomainSweeper:
    ...
    async def sweep(self) -> Dict[str, Any]:
        ...
        if not _CROSS_DOMAIN_ENABLE:
            result = {"success": True, "skipped": True, "reason": "disabled"}
            await self._log_pipeline(step="SWEEPER_SUMMARY", parsed=result)
            return result

        # 获取所有 L6 basic 节点。读取失败不能伪装成「无数据」。
        try:
            ...
            logger.error(f"[sweeper] Loading L6 basics failed: {e}", exc_info=True)
            return result
        basics = [
        ...
        if not basics:
            result = {"success": True, "skipped": True, "reason": "no L6 basics"}
            elapsed_ms = (datetime.now() - _t_start).total_seconds() * 1000
            await self._log_pipeline(
                step="SWEEPER_SUMMARY", parsed=result, elapsed_ms=elapsed_ms,
            )
        ...
        try:
            ...
            logger.error(f"[sweeper] Behavior embedding failed: {e}", exc_info=True)
            return result
        ...
        try:
            ...
            logger.error(f"[sweeper] Collision scan failed: {e}", exc_info=True)
            return result
        ...
        try:
            ...
            result = {
                ...
                "failed_cluster_ids": list(cluster_ids),
            }
            await self._log_pipeline(
                step="SWEEPER_SUMMARY",
                parsed=result,
                memory_ids=list(cluster_ids) + created_memory_ids,
                elapsed_ms=elapsed_ms,
            )
            logger.error(f"[sweeper] Cluster processing failed: {e}", exc_info=True)
            return result
    ...
    async def _ensure_beh_embeddings(self, basics: List[MemoryNode]) -> int:
        ...
        for node in basics:
            ...
            try:
                ...
                if not abstraction:
                    raise ValueError("LLM returned empty abstraction_for_embedding")

                # Embed → V_beh
                beh_embedding = await self.embed_service.embed_queued(abstraction)
                if not beh_embedding:
                    raise ValueError("embedding service returned an empty vector")

                # 写入 Graph 节点
                embedded = await self.graph_store.update_embedding(
                    node.node_id, beh_embedding=beh_embedding
                )
                if not embedded:
                    raise RuntimeError("behavior embedding write returned false")

                # 标记已生成 + 保存行为抽象文本
                custom["_has_beh"] = True
                custom["beh_abstraction"] = abstraction
                marked = await self.graph_store.update_node(
                    node.node_id, {"custom_json": json.dumps(custom, ensure_ascii=False)}
                )
                if not marked:
                    raise RuntimeError("behavior embedding marker write returned false")
                ...
                logger.error(
                    f"[sweeper] beh_embedding failed for {node.node_id[:12]}: {e}",
                    exc_info=True,
                )
                raise RuntimeError(
                    f"behavior embedding failed for {node.node_id}: {e}"
                ) from e
    ...
    async def _fetch_basics_with_beh(self) -> List[Dict[str, Any]]:
        ...
        rows = await self.graph_store.list_schema_embeddings(self._isolation_key)
        required = {"node_id", "content", "embedding", "beh_embedding"}
        normalized: List[Dict[str, Any]] = []
        for index, row in enumerate(rows):
            if not isinstance(row, dict):
                raise TypeError(
                    f"list_schema_embeddings row {index} must be dict, got {type(row).__name__}"
                )
            missing = required.difference(row)
            if missing:
                raise ValueError(
                    f"list_schema_embeddings row {index} missing fields: {sorted(missing)}"
                )
            if row["embedding"] is None or row["beh_embedding"] is None:
                raise ValueError(
                    f"list_schema_embeddings row {index} contains null embedding"
                )
    ...
    async def _process_cluster(
        ...
        if existing_core_ids:
            ...
            for nid in cluster_ids:
                ...
                if core_id not in targets:
                    linked = await self.graph_store.add_cross_abstracts_to(nid, core_id)
                    if not linked:
                        raise RuntimeError(
                            f"failed to verify CROSS_ABSTRACTS_TO {nid}->{core_id}"
                        )
        ...
        try:
            ...
            if await self.graph_store.get_node(core_id) is None:
                raise RuntimeError(f"core node postcondition failed: {core_id}")
            ...
            if inherited_evidence_ids:
                for vid in inherited_evidence_ids:
                    linked = await self.graph_store.add_derived_from(core_id, vid)
                    if not linked:
                        raise RuntimeError(
                            f"failed to verify inherited DERIVED_FROM {core_id}->{vid}"
                        )
                logger.debug(
            ...
            if core_id:
                try:
                    deleted = await self.graph_store.delete_node(core_id)
                    if not deleted:
            ...
            logger.error(f"[sweeper] Core creation failed: {e}{cleanup_error}")
            raise RuntimeError(f"core creation failed: {e}{cleanup_error}") from e
