...
# Only retry below the public 0.4 thresholds when every primary recall bucket is
# empty.  Keeping this fixed (rather than query/category-specific) makes the
# fallback conservative and benchmark-agnostic.
        ...
        try:
            ...
            with tracer.span("qdrant_search") as s:
                try:
                    ...
                    async def _search_profile(score_threshold: Optional[float] = None):
                        ...
                        if vdb_effective:
                            vdb_results = await vector_store.search(
                                ...
                                score_threshold=(
                                    profile_min_score if score_threshold is None else score_threshold
                                ),
                                only_latest=False,
                    ...
                    async def _search_proactive(score_threshold: Optional[float] = None):
                        ...
                        return await recall_intentions(
                            ...
                            limit=fetch_limit,
                            score_threshold=(
                                request.intention_min_score
                                if score_threshold is None else score_threshold
                            ),
                            now=request.as_of,
                        )

                    async def _search_normal(score_threshold: Optional[float] = None):
                        if layers_filter is not None:
                        ...
                        return await vector_store.search(
                            ...
                            score_threshold=(
                                (request.min_score if request.min_score > 0 else None)
                                if score_threshold is None else score_threshold
                            ),
                            only_latest=False,
                            created_after=request.created_after,
                    ...
                    if not (profile_results or proactive_results or normal_results):
                        zero_result_fallback_used = True
                        profile_results, proactive_results, normal_results = await asyncio.gather(
                            _search_profile(ZERO_RESULT_FALLBACK_MIN_SCORE),
                            _search_proactive(ZERO_RESULT_FALLBACK_MIN_SCORE),
                            _search_normal(ZERO_RESULT_FALLBACK_MIN_SCORE),
                        )
                        logger.info(
                            "[lite-read/legacy] zero-result fallback: min_score=%.1f "
                            "profile=%d proactive=%d normal=%d query='%s'",
                            ZERO_RESULT_FALLBACK_MIN_SCORE,
                            len(profile_results),
                            len(proactive_results),
                            len(normal_results),
                            request.query,
                        )
                    response.extra["zero_result_fallback"] = {
                        "used": zero_result_fallback_used,
                        "min_score": (
                            ZERO_RESULT_FALLBACK_MIN_SCORE
                            if zero_result_fallback_used else None
                        ),
                    }
