...
async def run_system2_agent(
    ...
    if operations is None:
        ...
        return {
            "success": False,
            "error": "failed to parse System 2 operations JSON",
            "tool_calls": [],
            "tool_call_log": [],
    ...
    # Execute operations via tool_executor.  create_schema operations may
    # expose a prompt-local ref; resolve those refs to the UUID returned by the
    # graph store before any later add_edge/add_evidence operation runs.
    all_tool_calls = []
    ...
    def _decode_result(raw: Any) -> Dict[str, Any]:
        ...
        if isinstance(raw, str):
            try:
                ...
                return {"success": False, "error": "tool returned invalid JSON", "raw": raw}
        return {"success": False, "error": "tool returned unsupported result type"}

    def _record(tool: str, op: Dict[str, Any], raw: Any, resolved_args: Dict[str, Any]) -> Dict[str, Any]:
        result_data = _decode_result(raw)
        ok = bool(result_data.get("success", "error" not in result_data)) and not result_data.get("error")
        entry = {
            "tool": tool,
            "args": op,
    ...
    for op in operations:
        ...
        try:
            if op_type == "create_schema":
                ...
                result = await tool_executor.execute("create_graph_node", resolved_args)
                result_data = _record("create_graph_node", op, result, resolved_args)
                if result_data.get("success") and result_data.get("node_id"):
                    created_index += 1
                    node_id = str(result_data["node_id"])
                    explicit_ref = str(op.get("ref") or "").strip()
                    if explicit_ref:
                        ref_map[explicit_ref] = node_id
                    # Backward compatibility for earlier prompts/LLM outputs
                    # that referred to creation order as schema_1/schema_id_1.
                    ref_map.setdefault(f"schema_{created_index}", node_id)
                    ref_map.setdefault(f"schema_id_{created_index}", node_id)

            elif op_type == "add_evidence":
                node_token = str(op.get("node_ref") or op.get("node_id") or "")
                resolved_args = {
                    "node_id": ref_map.get(node_token, node_token),
                    "evidence_list": op.get("evidence_list", []),
                }
                result = await tool_executor.execute("add_evidence", resolved_args)
                _record("add_evidence", op, result, resolved_args)

            elif op_type == "add_edge":
                source_token = str(op.get("source_ref") or op.get("source_id") or "")
                target_token = str(op.get("target_ref") or op.get("target_id") or "")
                resolved_args = {
                    "source_id": ref_map.get(source_token, source_token),
                    "target_id": ref_map.get(target_token, target_token),
                    "reason": op.get("reason", ""),
                }
                result = await tool_executor.execute("add_edge", resolved_args)
                _record("add_edge", op, result, resolved_args)

            else:
                logger.warning(f"[S2-agent] Unknown operation type: {op_type}")
                failure = {
                    "tool": op_type or "unknown",
                    "args": op,
                    "success": False,
                    "result": {"success": False, "error": "unknown operation type"},
                }
                all_tool_calls.append(failure)
                failures.append(failure)

        except Exception as e:
            logger.warning(f"[S2-agent] Failed to execute op {op_type}: {e}")
            failure = {"tool": op_type, "args": op, "success": False, "error": str(e)}
            all_tool_calls.append(failure)
            failures.append(failure)
    ...
    logger.info(
        ...
        f"{len(all_tool_calls)} executed, failures={len(failures)}, elapsed={elapsed_ms:.0f}ms"
    )

    return {
        "success": not failures,
        "error": "" if not failures else f"{len(failures)} System 2 operation(s) failed",
...
1. **create_schema** — 创建新的 L6 Schema；`ref` 是本次输出内唯一的临时引用
   ```{"op": "create_schema", "ref": "schema_1", "content": "当[场景]时，用户[模式]——反映了[洞察]。", "evidence_list": ["fact_node_id_1", "fact_node_id_2"], "tags": ["tag1"]}```

2. **add_evidence** — 给已有 Schema 追加证据（不修改内容）
   ```{"op": "add_evidence", "node_id": "existing_schema_node_id", "evidence_list": ["new_fact_id_1"]}```

3. **add_edge** — 建立两个 Schema 之间的关系。引用本次新建节点时必须使用 `source_ref`/`target_ref`；已有节点才使用真实 `source_id`/`target_id`
   ```{"op": "add_edge", "source_ref": "schema_1", "target_ref": "schema_2", "reason": "关系原因"}```
...
6. 每个 create_schema 的 ref 必须唯一；禁止把 ref 当作真实 UUID 写入 source_id/target_id。"""
    else:
        output_instructions = """
...
1. **create_schema** — Create a new L6 Schema; `ref` is a unique temporary reference within this output
   ```{"op": "create_schema", "ref": "schema_1", "content": "When [circumstance], the user [pattern] — reflecting [insight].", "evidence_list": ["fact_node_id_1", "fact_node_id_2"], "tags": ["tag1"]}```

2. **add_evidence** — Add evidence to an existing Schema (content stays immutable)
   ```{"op": "add_evidence", "node_id": "existing_schema_node_id", "evidence_list": ["new_fact_id_1"]}```

3. **add_edge** — Create a relationship between two Schemas. Use `source_ref`/`target_ref` for nodes created in this output; use real `source_id`/`target_id` only for existing nodes
   ```{"op": "add_edge", "source_ref": "schema_1", "target_ref": "schema_2", "reason": "relationship reason"}```
...
6. Every create_schema ref must be unique; never place a ref in source_id/target_id as though it were a UUID."""
