        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "source_ref": {"type": "string", "description": "Temporary ref returned by a create operation in the same plan"},
                    "target_ref": {"type": "string", "description": "Temporary ref returned by a create operation in the same plan"},
...
class System2ToolExecutor:
    ...
    async def execute(self, tool_name: str, args: Dict[str, Any]) -> str:
        ...
        self.tool_call_log.append({
            "tool": tool_name,
            "args": args,
            # Keep the structured result in logs so callers can reliably
            # aggregate success/failure.  The return value remains a JSON
            # string for backward compatibility with the old tool loop.
            "result": result,
    ...
    async def _tool_create_graph_node(self, args: Dict) -> Any:
        ...
        if layer not in (MemoryLayer.L6_SCHEMA, MemoryLayer.L7_INTENTION):
            return {
                "success": False,
                "created": False,
                "error": f"unsupported graph layer: {layer_str}",
            }

        # Validate every evidence reference before creating anything.  The
        # previous implementation created orphan VdbRef nodes for unknown IDs
        # and still reported the full requested evidence_count.
        evidence_nodes: Dict[str, MemoryNode] = {}
        invalid_evidence: List[Dict[str, str]] = []
        for eid in evidence_list:
            vdb_node = await self.vector_store.get_by_id(eid)
            if vdb_node is None:
                invalid_evidence.append({"node_id": eid, "reason": "not_found"})
                continue
            if vdb_node.user_id != self.user_id or vdb_node.agent_id != self.agent_id:
                invalid_evidence.append({"node_id": eid, "reason": "isolation_mismatch"})
                continue
            evidence_nodes[eid] = vdb_node
        if invalid_evidence:
            return {
                "success": False,
                "created": False,
        ...
        stored_node_id = await self.graph_store.upsert_memory_node(node)
        persisted = await self.graph_store.get_node(stored_node_id)
        if persisted is None:
            return {
                "success": False,
                "created": False,
                "node_id": stored_node_id,
                "error": "graph node postcondition failed",
            }

        # Create VdbRef + DERIVED_FROM edges for each evidence
        evidence_added = 0
        for eid in evidence_list:
            vdb_node = evidence_nodes[eid]
            ev_layer = vdb_node.layer.value
            await self.graph_store.ensure_vdbref(eid, ev_layer)
            linked = await self.graph_store.add_derived_from(node_id, eid)
            if not linked:
                # Compensating cleanup: this node ID was generated in this
                # method, so removing it cannot affect pre-existing data.
                await self.graph_store.delete_node(node_id)
                return {
                    "success": False,
                    "created": False,
                    "node_id": node_id,
                    "error": f"DERIVED_FROM postcondition failed for evidence {eid}",
    ...
    async def _tool_add_evidence(self, args: Dict) -> Any:
        ...
        if graph_node is None:
            return {"success": False, "node_id": node_id, "error": "graph node not found"}
        if graph_node.user_id != self.user_id or graph_node.agent_id != self.agent_id:
            return {"success": False, "node_id": node_id, "error": "graph node isolation mismatch"}
        ...
        for eid in evidence_list:
            ...
            if vdb_node is None:
                invalid_evidence.append({"node_id": eid, "reason": "not_found"})
            elif vdb_node.user_id != self.user_id or vdb_node.agent_id != self.agent_id:
                invalid_evidence.append({"node_id": eid, "reason": "isolation_mismatch"})
            else:
                evidence_nodes[eid] = vdb_node
        if invalid_evidence:
        ...
        return {"success": True, "node_id": node_id, "evidence_added": added, "verified": True}
    ...
    async def _tool_add_edge(self, args: Dict) -> Any:
        ...
        if source_id == target_id:
            return {
                "success": False,
                "verified": False,
                "error": "self-referential RELATED_TO edges are not allowed",
            }

        source = await self.graph_store.get_node(source_id)
        target = await self.graph_store.get_node(target_id)
        missing = [
            node_id
            for node_id, node in ((source_id, source), (target_id, target))
            if node is None
        ]
        if missing:
            return {
        ...
        if (
            source.user_id != self.user_id
            or target.user_id != self.user_id
            or source.agent_id != self.agent_id
            or target.agent_id != self.agent_id
        ):
            return {
                "success": False,
                "verified": False,
                "error": "edge endpoint isolation mismatch",
            }
        allowed_layers = {MemoryLayer.L6_SCHEMA, MemoryLayer.L7_INTENTION}
        if source.layer not in allowed_layers or target.layer not in allowed_layers:
            return {
                "success": False,
                "verified": False,
                "error": "RELATED_TO endpoints must be L6/L7 graph nodes",
            }
        ...
        return {
            ...
            "source_id": source_id,
            "target_id": target_id,
            "edge": f"({source_id})-[RELATED_TO]->({target_id})",
            **({} if success else {"error": "RELATED_TO write postcondition failed"}),
        }
