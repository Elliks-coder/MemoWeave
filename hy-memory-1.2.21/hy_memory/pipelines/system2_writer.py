...
class System2Writer(WritePipeline):
    ...
    async def write_extracted(
        ...
        """已抽取事实直写：委托给内部 MemoryWriter.write_extracted（跳过 extract）。

        System 2 不在此触发——与 write() 一致，认知加工由 digest() 显式触发。
        """
        if not self._initialized:
            await self.initialize()
        response = await self._writer.write_extracted(request, ctx=ctx, tracer=tracer)
        response.extra["pipeline_version"] = "system2"
        response.extra["system2_task_ids"] = []
        return response
                ...
                if tool_name in ("create_graph_node", "update_graph_node", "delete_graph_node", "add_edge"):
                    try:
                        ...
                        if isinstance(result_data, str):
                            try:
                                result_data = _json_s2.loads(result_data)
                            except Exception:
                                result_data = {}
                        # Failed attempts remain visible in the trajectory, but
                        # must not be persisted as successful graph operations.
                        if not result_data.get("success", "error" not in result_data):
                            continue
    ...
    async def _run_cross_domain_sweeper(
        ...
        if not _SWEEPER_ENABLED:
            return {"success": True, "skipped": True, "reason": "SWEEPER_ENABLED=false"}
    ...
    async def digest(
        ...
        agent_result = (results or {}).get("system2_agent", {})
        agent_success = bool(agent_result.get("success", False))
        sweeper_success = bool(sweeper_result.get("success", False))

        return {
            "success": agent_success and sweeper_success,
            "request_id": request_id,
            "tasks_processed": 1,
            "results": results,
            ...
            "errors": [
                error
                for error in (
                    agent_result.get("error") if not agent_success else "",
                    sweeper_result.get("error") if not sweeper_success else "",
                )
                if error
            ],
        }
